...
<head>
    ...
    <style>
        ...
        .content table td .status-button {
            border: none;
            border-radius: 10px;
            color: black;
            background-color: rgb(195, 195, 73);
            width: 100px;
        }
		.hidd-inp{
		    display: none;
...
<body>
    ...
    <div class="create-heading">
        <h2>**Please approve fund transfers wisely**</h2>
    </div>
    <div>
        <form action="/DisplayFund" method="post" class="form1" onsubmit="return validate()">
			...
			<%
			    ...
			    if (Message != null) {
			        if (Message.equals("content")) {
			            ...
			            long rid = (Long) request.getAttribute("rid"); 
			            ...
			            <div class="content">
			                <table>
			                    ...
			                    <tr>
									...
									<td><%=status %></td>
									<td>
									    <form action="\FundApproved">
											<input type="number" value="<%=uid%>" class="hidd-inp" name="uid">
											<input type="number" value="<%=rid%>" class="hidd-inp" name="rid">
											<input type="number" value="<%=amount%>" class="hidd-inp" name="amount">
									        <input type="submit" value="Approve" class="status-button">
									    </form>
									</td>
                                    <td>
									    <form action="\FundRejected">
											<input type="number" value="<%=uid%>" class="hidd-inp" name="uid">
											<input type="number" value="<%=rid%>" class="hidd-inp" name="rid">
											<input type="number" value="<%=amount%>" class="hidd-inp" name="amount">
									        <input type="submit" value="Reject" class="status-button" id = "button2">
									    </form>
                                    </td>
			                    </tr>
			                </table>
			...
			%>
			            <p class="message" id="message1">There is no Fund Trnasfers with given details</p>
			<%
			        }
					else if (Message.equals("success")){
			%>
						<p class="message" id="message2">Fund Successfully approved</p>
			<%
					}
					else if (Message.equals("rejected")){
			%>
						<p class="message" id="message1">Fund Successfully rejected</p>
			<%
					}
			    }
			%>
